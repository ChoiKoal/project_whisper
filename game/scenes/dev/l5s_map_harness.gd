...
func _test_counts(loader: MapLoader, zone: String) -> void:
	...
	# (v1.9.1 §㉙) 종의 들판 격자 과밀 개선: A/Q/C/O 맨 바닥 셀의 30.1%를 '침묵한 종들의 무덤'
	#   장식(b/m/p/u/t)으로 치환 + 대각선 빈 회랑(f) 3선으로 격자 절단. 보행성 불변(치환·회랑 모두
	#   L5T-C 워크블 바닥, blocks=false → walkable 696 그대로). 게이트/채집/랜드마크 셀 무변경.
	#   V(900) 바래 사라진 허공 · A(125) 착지 상아 바닥 · Q(141) 종실 회랑 · C(129) 타종 울림 회랑 ·
	#     O(48) 종탑 정점 · /(4) 경사로.  (A/Q/C/O = §㉙ 치환 후 잔여 맨 바닥 수.)
	#   §㉙ 장식(무발광/희미, 비채집 l5xobj): b(37) 깨진 받침 · m(33) 무명석 · p(17) 빈 받침 ·
	#     u(34) 낮은 잔해 · t(18) 기울어진 종.  f(49) 대각선 빈 회랑 바닥(오브젝트 無).
	...
	var expect := {"V": 900, "A": 125, "Q": 141, "C": 129, "O": 48, "/": 4,
		"b": 37, "m": 33, "p": 17, "u": 34, "t": 18, "f": 49,
