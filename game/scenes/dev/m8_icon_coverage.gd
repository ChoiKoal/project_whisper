...
func _ready() -> void:
	...
	_check("68 Layer-1 canonical + 48 Layer-2 + 1 alias split",
		canonical_ids.size() == 68 and l2_ids.size() == 48 and alias_ids.size() == 1)

	# 1. every canonical id (Layer-1 AND Layer-2) has a real icon FILE (present on disk).
	var missing: Array[String] = []
	for id in canonical_ids + l2_ids:
		if not ResourceLoader.exists(ICON_DIR + id + ".png"):
			missing.append(id)
	_check("every canonical id (L1+L2) has an icon PNG (missing=%s)" % [missing], missing.is_empty())

	# 2. ItemDB.icon(id) non-null for every id (canonical + L2 + alias).
	var null_ids: Array[String] = []
	for id in canonical_ids + l2_ids + alias_ids:
		if ItemDB.icon(id) == null:
			null_ids.append(id)
	_check("ItemDB.icon() non-null for all ids (null=%s)" % [null_ids], null_ids.is_empty())

	# 2b. Layer-2 icons resolve to a REAL painted file, not the category fallback square.
	# (ItemDB.icon returns a fallback ImageTexture when the PNG is absent; assert the PNG exists
	# AND load() succeeds so L2 items are genuinely covered, not silently squared.)
	...
	_check("all 48 Layer-2 icons are real painted files (fallback=%s)" % [l2_fallback], l2_fallback.is_empty())
	...
	# 3. no two icon files byte-identical (116 canonical files all unique: 68 L1 + 48 L2).
	var hashes := {}
	var dup_pairs: Array = []
	for id in canonical_ids + l2_ids:
		...
		if hashes.has(h):
			...
			hashes[h] = id
	_check("all 116 canonical icon files are byte-unique (dupes=%s)" % [dup_pairs], dup_pairs.is_empty())
	_check("distinct icon hashes == 116", hashes.size() == 116)
