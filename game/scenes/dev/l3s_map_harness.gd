extends Node
## (EXL3-6) L3 확장 SUB-zone 「태엽 광산」(l3m) map/scene acceptance harness.
##
## Boots the real scene (clockwork_mine.tscn — the scene the 낡은 광차 승강로 하강 travels to) and
...
##   3. all-gates-open walkable count = 699 per design §A-6.1 (l3x_bfs.py).
##   4. every gate / landmark / gatherable / gate-object cell exists at the expected coord.
##   5. every authored l3m object instantiated with a real texture (no black-box sprites).
...
func _ready() -> void:
	print("=== L3S MAP HARNESS (태엽 광산 l3m) ===")
	_reset_autoloads()
	await _test_zone(MINE, "mine")
...
func _test_counts(loader: MapLoader, zone: String) -> void:
	...
	# Authoritative l3m layout inventory (l3x_map_gen.py). GM1 K(2), GM2 D(2), GM3 M(2)+x(3),
	# GM4 H(1)+O(1). 채집 h(10)/k(10)/o(10)/b(7). E(1) 잔류 태엽 발전기. N(1) 굴착 로봇.
	# C(1) 정비대. ~(44) 붕락 낙석 협곡. (l2s와 동형이나 C 정비대 심볼로 P 183.)
	var expect := {"V": 857, "B": 71, "G": 389, "P": 183, "~": 44, "K": 2, "D": 2, "M": 2,
		"x": 3, "H": 1, "O": 1, "E": 1, "N": 1, "C": 1, "S": 1, "h": 10, "k": 10, "o": 10, "b": 7}
	var ok := true
	for sym in expect:
...
## All-gates-open walkable = every cell that is not void (V) and not 붕락 낙석 협곡 (~ 무너진 암반).
...
func _test_objects(loader: MapLoader, zone: String) -> void:
	_check("%s l3m objects spawned" % zone, loader.l2_object_nodes.size() > 0,
	...
	_check("%s vent_door + excavator_altar + excavator_core + residual_dynamo spawned" % zone,
		seen.has("vent_door") and seen.has("excavator_altar") and seen.has("excavator_core") \
		and seen.has("mine_residual_dynamo"))
	_check("%s K12 unique 채집원(excavator_core) spawned" % zone, seen.has("excavator_core"))
