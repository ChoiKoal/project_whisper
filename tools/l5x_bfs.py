...
# -*- coding: utf-8 -*-
"""
l5x_bfs.py — EX-L5 구역 「침묵의 종탑」 게이트 순서 강제 BFS 증명.

스폰에서 4방향 BFS. 벽 = void(V=바래 사라진 허공/종탑 층 사이) + '현재 닫힌' 게이트 병목 셀
...
l4x_bfs.py 문법 계승(단일 구역판) + 균열 검증(severed) 계승. GB1~GB4 게이트로 적응.

재현: python3 tools/l5x_map_gen.py && python3 tools/l5x_bfs.py
"""
import json
import os
...
def audit(layer, gate_order, zone_probes, title):
    ...
    for label, opened in states:
        # 부적(GB3 부적)이 열린 상태부터 ward=True(균열/L 통과). 순서 강제는 병목이 담당.
        ward = "GB3" in opened
...
def main():
    print("========================================================")
    print(" EX-L5 SPATIAL ORDER PROOF — 침묵의 종탑")
    print("========================================================")
    # GB1(무너진 종탑 계단)→GB2(흐려진 종음 결계)→GB3(타종 울림 순서 퍼즐 상층문)→GB4(큰 종 재타종)
    # probes: spawn / lower(GB1북) / puzzle(GB2북 퍼즐실) / top-approach(GB3북) / altar(GB4목)
    probes = {
        "spawn": [19, 39],
        "lower(GB1북)": [19, 28],
        "puzzle(GB2북)": [19, 16],
    ...
    ok, w = audit("l5x", ["GB1", "GB2", "GB3", "GB4"], probes, "침묵의 종탑")

    print("\n========================================================")
    print(f" 침묵의 종탑 : {'PASS' if ok else 'FAIL'}  (walkable {w})")
