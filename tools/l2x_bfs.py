...
# -*- coding: utf-8 -*-
"""
l2x_bfs.py — EX-L2 구역 「지하 데이터 성소」 게이트 순서 강제 BFS 증명.

스폰에서 4방향 BFS. 벽 = void(V) + 물(~ 냉각 침수로) + '현재 닫힌' 게이트 병목 셀.
...
l1x_bfs.py 문법 계승(단일 구역판).

재현: python3 tools/l2x_map_gen.py && python3 tools/l2x_bfs.py
"""
import json
import os
...
def main():
    print("========================================================")
    print(" EX-L2 SPATIAL ORDER PROOF — 지하 데이터 성소")
    print("========================================================")
    # GB1(냉각 침수로)→GB2(봉인 격벽)→GB3(데이터 문)→GB4(백업 봉헌)
    # probes: spawn / corridor(GB1북) / align(GB2북 정합실) / sanctum(GB3북 사원) / altar(GB4목)
    probes = {
        "spawn": [19, 39],
        "corridor(GB1북)": [19, 28],
        "align(GB2북)": [19, 16],
    ...
    ok, w = audit("l2s", ["GB1", "GB2", "GB3", "GB4"], probes, "지하 데이터 성소")

    print("\n========================================================")
    print(f" 지하 데이터 성소 : {'PASS' if ok else 'FAIL'}  (walkable {w})")
