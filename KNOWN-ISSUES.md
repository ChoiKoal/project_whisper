# Project Whisper — 알려진 이슈 (v0.1.0, M6b 기준)
...
- **없음(블로커 기준).** 아래는 진행에 영향 *가능성*이 있어 진행 카테고리로 관찰만 하는 항목:
  - **G1 개울은 3칸 깊이 — 디딤돌 3개 필요.** K 슬롯이 세로로 3칸(예: (16,24)/(16,25)/(16,26))이라
    디딤돌 하나로는 건널 수 없다. 이건 의도된 난이도지만, **디딤돌이 3개 필요하다는 안내(툴팁/힌트)가
    아직 없어서** 플레이어가 하나만 놓고 "왜 못 건너지?" 하고 막힐 여지가 있다. → 인게임 힌트 필요(체감/진행 경계).
    (E2E 하니스는 3개를 놓고 건너는 경로까지 검증함.)
...
- **밤 글로우 틴트 블리드.** 세계수/신비한 물/밤 꽃봉오리의 additive GlowSprite 가 CanvasModulate(DayNight)
  와 같은 캔버스에 있어, 밤 색보정이 글로우에도 살짝 물든다. 밤의 "확 빛나는" 느낌이 기획만큼 강하지 않음.
  → 전용 CanvasLayer 로 분리하면 정확해짐(M4 핸드오프 후속 항목).
- **G1 다중 디딤돌 안내 부재** (위 🔴 관찰 항목과 동일 원인, 체감 측면).
