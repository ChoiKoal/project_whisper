...
갱신: 2026-07-08 · v1.2.0 — 아트 정합 패치(AP-1~4). 전 오브젝트 아이소메트릭 재작업 + 캐릭터 8방향 + 솥단지 리디자인. 아래 "✅ 해결됨(v1.2.0)" 절 참조 — 「🟢 외관」 플레이스홀더 아트/라이팅 방향 불일치 대거 해소.
갱신: 2026-07-08 · v1.3.0 — 컷신 연출 패치(CQ). 오프닝 아크(각성 심장박동·포탈 리빌·세계수 조우) 신설 + 세계별 정화 연출 5종 + 엔딩 폴리시(E2 보라 점 리듬 회수) + 컷신 재감상 갤러리(도감 「기록」 탭). 아래 "✅ 해결됨(v1.3.0)" 절 참조 — 「🟡 연출」 카드-일변도 컷신·공통 부품 5중 중복 해소.
갱신: 2026-07-08 · v1.3.1 — 세이브/진행 회귀 2건 수정(BUG A: L1 재방문 시 L2 과학 포탈 재잠금 / BUG B: 정화한 숲 덤불 재입장 시 리셋) + 잠긴 세이브 로드 시 자동 복구. 아래 "✅ 해결됨(v1.3.1)" 절 참조 — 「🔴 세이브」 진행 회귀 데드락 위험 해소.
갱신: 2026-07-10 · v1.4.1 — KOAL 실플레이 리포트 3건 수정(BUG 1: 렌더↔인게임 맵 방위 불일치[컴포지터 diamond→STACKED 투영 정합, 전 레이어] / BUG 2: 솥단지 발광 오브 "동그라미 둥둥"[개구부·몸통 목 간극 제거] / BUG 3: 벽 뒤 나무 뚫림[릿지 벽 YSort 편입·접지 앵커]). 아래 "✅ 해결됨(v1.4.1)" 절 참조.
갱신: 2026-07-10 · v1.4.2 — L0 홈 섬 레이아웃 재저작(staggered 투영 기준, 21×17). 옛 diamond 투영을 가정해 저작된 `home_layout.txt`가 실제 staggered 렌더에서 "좌측 뭉침"이던 것을 의도된 **포탈 아치**(P3 top-center, P1↔P5·P2↔P4 X대칭)로 복원. **세이브 호환**: 구 좌표가 새 슬랩의 VOID/밖에 착지 시 로드 시 최근접 보행 셀로 자동 clamp(`save_manager._nearest_walkable_world`, "이미 보행 가능이면 무변경"이라 기존 세이브 회귀0). 하네스 정합: e2e/v050c/m7 홈 차원 임계값 갱신(22×22→21×17; m7 flow watcher `_grove_ok` height≥20→≥15 — 21×17의 height 17이 옛 임계 20에 걸려 무한 대기하던 하네스-전용 회귀). **게임 로직/충돌 스키마 무변경**(레이아웃 데이터 + 로드 좌표 clamp뿐).

---

## ✅ 해결됨 (v1.4.1) — KOAL 실플레이 리포트 3건 (렌더 방위·솥 이펙트·가림 순서)
- 🔴 **[해결] BUG 1: 렌더 ↔ 인게임 맵 방위 불일치 (전 레이어).** 오버뷰 프리뷰(tools_overview_*.js)의 맵이 인게임과 다른 방향으로 보였다(L0 홈·L1 숲 확인, 전 레이어 해당). **원인 = 투영 공식 불일치**: 게임 타일셋(`whisper_tileset.tres`)은 아이소메트릭 **tile_layout=0(STACKED)** + **tile_offset_axis=0(HORIZONTAL)** 128×64라 Godot `map_to_local`이 **staggered/offset 배치**(col=화면 수평, row=반칸 stagger)인데, 6개 컴포지터는 **diamond 공식** `[(c-r)·64,(c+r)·32]`을 써서 맵이 **~45° 회전(축 반사 포함)**돼 있었다. **정정 원칙(게임 데이터+인게임 투영=정본)**대로 컴포지터를 게임에 일치: `cellLocal`을 STACKED로 교체(`[(c+0.5·(r&1))·128, r·32]`) — 6종 전부(home/l1~l5). 부수로 오브젝트 y-sort 키를 `c+r`(diamond depth) → 화면 Y(`baseY`)로 교정(STACKED에선 화면Y가 row만의 함수). home 중심/데브리 좌표도 `cellLocal`화. **판정 결과**: (b)인게임 투영 ↔ (c)컴포지터 쌍이 뒤집혀 있었고(diamond vs STACKED = 45° 회전), (a)레이아웃 데이터와 (b)인게임은 정합이었다 → 컴포지터만 수정. 재렌더 후 랜드마크 정합 확인 — L1: 세계수·연못 top-center(북)·강 수평 밴드; L2: 관제탑 북/스폰 남(수직 여정); L3 시계탑·L5 종탑 북. **게임 코드 무변경**(세이브·하네스 영향 0).
...
## ✅ 해결됨 (v1.3.1) — 세이브/진행 회귀 수정 (save regression fix)
- 🔴 **[해결] BUG A: L1 재방문 시 L2 포탈 재잠금.** L1(자연 숲)을 클리어해 science 포탈이 OPEN 된 뒤, L1을 다시 방문했다가 집으로 돌아오면 홈의 스냅샷이 진행 플래그보다 오래되어 science 포탈이 DORMANT로 되돌아가 진행이 막혔다. **원인**: 홈/월드 부팅이 스냅샷을 무조건 신뢰해 정화 완료 세계의 포탈을 재잠금. **수정**: `game/scripts/core/game_state.gd`에 `reconcile_portal_line()`(진행 플래그 기준 단일 소스 재계산, UPGRADE 전용·idempotent) 신설 — 매 월드 부팅 시 호출(각 world_session). CS-05 점화 비트 이후로 치유를 지연(`game/scripts/world/home_session.gd:~109`)해 오프닝 리빌과 충돌 없음.
- 🔴 **[해결] BUG B: 정화한 숲 덤불 재입장 리셋.** 물을 준 숲 덤불이 숲을 나갔다 재입장(포탈 왕복)하면 원상태로 리셋됐다. **원인**: 인-런 재입장 경로가 등록된 월드 스냅샷을 복원하지 않음. **수정**: `game/scripts/core/save_manager.gd:440` `restore_registered_world()` — 재입장 시 현재 씬의 인-메모리 월드 스냅샷 복원(`game/scripts/world/grove_session.gd:62`).
- 🔴 **[해결] 잠긴 세이브 자동 복구(마이그레이션).** 위 버그로 이미 science/nature 포탈이 잠긴 채 디스크에 저장된 세이브도, 로드 시 `reconcile_portal_line()`이 진행 플래그(SaveManager.cleared / layer2~5_purified_flag) 기준으로 포탈을 재계산해 OPEN으로 자동 복구(별도 마이그레이션 스텝 불필요). 세이브 스키마 무변경 → 100% 호환.
- 검증: `v131_saveregress_harness`(BUG A/B + 잠긴 세이브 자동 복구 3파트, 재현→픽스 후 9 PASS 0 fail) + 전 하네스 스위프 44/44 그린 + 실 PCK(--main-pack) v131/e2e PASS.

---

## ✅ 해결됨 (v1.3.0) — 컷신 연출 패치 (cutscene quality-up)
- 🟡 **[해결] 카드 일변도 컷신 → 실연출.** 오프닝(opening.gd)이 텍스트 카드 4장만이었고(보라 점/심장박동/실루엣/포탈 리빌 전무), CS-03 세계수 조우 컷신 자체가 미구현이었다. **수정**: 각성 심장박동(보라 점 2회 + 낮은 단음) + 오프닝→홈 각성 리빌(줌아웃 포탈 반원 리빌) + CS-03 세계수 조우(카메라 틸트 + BGM 저음 + 잎 기욺) 신설.
- 🟡 **[해결] 세계별 정화 set piece 무차별.** L2~L5 각 gate_controller가 로컬 카드+웨이브 점등만 재구현(링/플래시/카메라 없음, 톤 차별화 없음). **수정**: 공통 CutsceneDirector 부품화 + 세계별 톤 연출 5종(L1 파문 링/L2 도시 점등/L3 대시계 첫 틱·톱니/L4 봉인 수축/L5 잔불→온기 확산).
- 🟡 **[해결] 공통 부품 5중 중복.** 카메라 팬/줌/틸트 Tween·레터박스·화이트 플래시·확장 링 파문·카드 타이포가 opening/clear/portal/ending/l2~l5에 5중 중복. **수정**: CutsceneDirector 유틸로 추출·재사용. 엔딩 E2 보라 점 리듬을 오프닝 상수와 공유(리듬 회수).
- 🟢 **[해결] 컷신 재감상 부재.** 도감 「기록」 탭이 진상 조각만 노출·재감상 전무. **수정**: 열람한 컷신 재감상 갤러리 신설(도감 「기록」 탭).
- 검증: 전 하네스 스위프 43/43 그린 + 실 PCK(--main-pack) cutscene_harness(잠긴 컷신 재생 거부 포함)/e2e_playthrough PASS. **세이브 100% 호환**.
- 릴리스: v1.3.0 (win64 40,087,353 / macos 68,419,826 bytes, mac ad-hoc 서명 verify OK). URL: https://github.com/ChoiKoal/project_whisper/releases/tag/v1.3.0

---

## ✅ 해결됨 (v1.2.0) — 아트 정합 패치 (isometric coherence)
- **전 오브젝트 아이소메트릭 재작업**: 6개 맵(홈/L1~L5) 100+종 오브젝트를 정면뷰(front-view)에서 3/4 원근으로 통일. 캐릭터 8방향 이동 시트(character_sheet.png 288×768). 솥단지(cauldron) 리디자인.
- 「🟢 외관」의 **플레이스홀더 아트 전반** + **라이팅 방향 불일치**(우상단/좌상단 혼재)를 아이소 재작업으로 대거 해소. 잔여 절차생성 아트는 art-style-guide 기준으로 통일.
- 검증: 전 하네스 스위프 35/35 그린 + 실 PCK(--main-pack) e2e/ap3_facing/interaction_fusion 3종 PASS + 컴포지터 프리뷰 6종 재생성(새 아이소 아트 반영). **세이브 100% 호환**.
- 릴리스: v1.2.0 (win64 40,059,768 / macos 68,392,669 bytes). URL: https://github.com/ChoiKoal/project_whisper/releases/tag/v1.2.0
